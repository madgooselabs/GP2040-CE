...
export const mpr121Scheme = {
	MPR121AddonEnabled: yup
		.number()
		.required()
		.label('MPR121 IO Add-On Enabled'),
};

export const mpr121State = {
	MPR121AddonEnabled: 0,
};
			...
			<div className="d-flex col py-2">
				...
				<CustomSelect
					...
					getOptionLabel={(option) => {
						...
						return t(`MPR121:label-${labelKey}`);
					}}
					onChange={(change) =>
						setPinDirection(
							expansionType,
...
const MPR121 = ({ values, errors, handleChange, handleCheckbox }) => {
	...
	return (
		<Section title={t('MPR121:header-text')}>
			<div
				id="MPR121AddonOptions"
				hidden={!(values.MPR121AddonEnabled && getAvailablePeripherals('i2c'))}
			>
				<Row className="mb-2">
					<ExpansionPinsForm
						pins={pins}
			...
			<FormCheck
				...
				type="switch"
				id="MPR121AddonButton"
				reverse
				isInvalid={false}
				checked={Boolean(values.MPR121AddonEnabled)}
				onChange={(e) => {
					handleCheckbox('MPR121AddonEnabled', values);
					handleChange(e);
				}}
			/>
		</Section>
