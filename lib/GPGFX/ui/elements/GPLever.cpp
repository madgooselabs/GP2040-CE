...
void GPLever::draw() {
    ...
    if (this->_inputType == 0) {
        ...
        uint16_t minX = max(0,(baseX - baseRadius));
        uint16_t maxX = min((baseX + baseRadius),128);
        uint16_t offsetX = (analogX * 0.01) * (maxX - minX);
        uint16_t minY = max(0,(baseY - baseRadius));
        uint16_t maxY = min((baseY + baseRadius),64);
    ...
    if (this->_showCardinal) {
        ...
        uint16_t cardinalN = max(0,(baseY - baseRadius)-cardinalSize);
        uint16_t cardinalS = min((baseY + baseRadius),64);
        uint16_t cardinalE = max(0,(baseX - baseRadius)-cardinalSize);
        uint16_t cardinalW = min((baseX + baseRadius),128);
