#include "addons/i2c_gpio_mpr121.h"
#include "storagemanager.h"
#include "helper.h"
#include "config.pb.h"

bool MPR121Addon::available() {
    const MPR121Options& options = Storage::getInstance().getAddonOptions().mpr121Options;
    if (options.enabled) {
        mpr = new MPR121();
        PeripheralI2CScanResult result = PeripheralManager::getInstance().scanForI2CDevice(mpr->getDeviceAddresses());
        if (result.address > -1) {
            mpr->setAddress(result.address);
            mpr->setI2C(PeripheralManager::getInstance().getI2C(result.block));
            return true;
        } else {
            delete mpr;
        }
